...
{
  string tempFile="";//Edited by chengzj;
  for(int i=0; i<input.size(); i++)
  {
    if(input[i].find(".json",0)!=-1){//Edited by chengzj;
       if(tempFile!=input[i]){
...
StatusCode RootInterface::createTree(const unsigned int treenr,const std::string treename)
{
  ...
  m_outputFiles[treenr] =new TFile(m_outputFilenames[treenr].c_str(), "RECREATE");
...
TTree * RootInterface::getTree(const std::string treename)
{
  ...
  // get TTree for input
  log<<MSG::INFO<<"RootInterface:;getTree treename is "<<treename<<endreq; 
  unsigned int treenr;
  getTreeNr(treename,treenr);
  log<<MSG::INFO<<"RootInterface:;getTree treenr is "<<treenr<<endreq; 
  ...
  if (!m_inputFiles[treenr] ) {
   ...
   log<<MSG::INFO<<"RootInterface:;getTree m_fileNames[treenr] is: "<<m_fileNames[treenr].c_str()<<endreq; 
...
StatusCode RootInterface::f_createTree(unsigned int treenr,
    ...
    m_single_outputFiles[treenr] =
        new TFile(m_single_outputFileNames[treenr].c_str(), "RECREATE");
